...
- 여러 URL을 queue에 추가하고 입력 순서대로 자동 처리
- 대기 중인 queue 항목 삭제 및 항목별 진행 상태 표시
...
- URL, 인식 텍스트, 언어, 모델 정보를 JSON 결과물로 영구 저장
- 이미 변환된 URL은 디스크 결과를 재사용하고 해당 결과 카드를 하이라이트
- 결과 카드에서 언어를 변경해 강제 재실행
- 결과 텍스트 복사 및 JSON 결과 파일 열기
- 출력 폴더 직접 선택
...
5. YouTube URL을 입력하고 `Queue에 추가`를 누릅니다. URL을 더 입력해 연속 작업을 예약할 수 있습니다.
6. 오른쪽 queue 패널에서 처리 순서와 진행률을 확인합니다. 처리 중이지 않은 항목은 X 버튼으로 제거할 수 있습니다.
7. queue 아래 실시간 Transcript에서 생성되는 텍스트를 확인합니다.
8. 변환이 끝나면 결과 카드에서 텍스트를 복사하거나 JSON 파일을 엽니다.
9. 다른 언어로 다시 인식하려면 결과 카드의 언어를 변경하고 `재실행`을 누릅니다.

각 변환은 출력 폴더에 `*.hushline.json`으로 저장됩니다. 같은 URL을 다시 추가하면 STT를 반복하지 않고 저장된 JSON 결과를 불러오며, 해당 결과 카드가 잠시 강조됩니다. 결과 카드의 언어 변경 재실행은 이 중복 방지를 의도적으로 우회하고 기존 결과를 새 결과로 갱신합니다.
...
│   ├── queue-panel/             # 순차 작업 queue와 실시간 Transcript
│   ├── result-grid/             # 저장 결과 카드와 언어 변경 재실행
│   └── transcript-panel/        # Transcript 프레젠테이션 컴포넌트
